...
export default function CuestionarioOSIVQ() {
  const { enviarOSIVQ, osivqCompletado, cuestionarioCompletado, getMisSesiones, limpiarOsivqTemp, limpiarCuestionarioTemp } = useAuth()
  ...
  // Cargar sesiones una vez para comparar fechas
  useState(() => {
    setLoadingSes(true)
    getMisSesiones()
  ...
  // Esperar que carguen las sesiones
  if (loadingSes || sesiones === null) {
    return <div style={{ padding: '40px', color: 'var(--muted)', textAlign: 'center' }}>Cargando...</div>
  }

  // ── Bloqueo 2: hay sesión biométrica más reciente que el F-S ─────────────
  // Significa que ya completó el ciclo y debe rehacer el F-S primero
  if (!rehaciendo && sesiones.length > 0) {
    const fechaFS           = new Date(cuestionarioCompletado.fecha)
    const fechaUltimaSesion = new Date(sesiones[0].fecha)
    if (fechaUltimaSesion > fechaFS) {
      return (
        <div style={styles.container}>
          <div style={styles.tag}>V-COGNI · Cuestionario OSIVQ</div>
          <div style={styles.card}>
            <div style={styles.bloqIcon}>🔒</div>
            <h2 style={styles.heroTitle}>Primero actualiza el Cuestionario F-S</h2>
            <p style={styles.heroDesc}>
              Ya realizaste una prueba biométrica después de tu último cuestionario.
              Para volver a hacer el OSIVQ, debes responder primero el Cuestionario F-S nuevamente.
            </p>
            <button style={styles.irSistemaBtn} onClick={() => navigate('/cuestionario')}>
              Ir al Cuestionario F-S →
            </button>
        ...
        {showConfirm && (
          <div style={styles.overlayConfirm}>
            <div style={styles.modalConfirm}>
              ...
              <p style={styles.modalDesc}>Deberás completar el flujo completo desde el inicio: F-S → OSIVQ → Prueba biométrica.</p>
        ...
        return (
          <div key={item.id} style={{ ...styles.pregCard, ...(resp ? styles.pregCardRespondida : {}) }}>
            <div style={styles.pregHeader}>
              ...
              {item.invertido && <div style={styles.invertidoTag}>↩ invertida</div>}
            </div>
            <div style={styles.escalaWrap}>
              {ESCALA.map(e => (
                <button
                  key={e.valor}
                  style={{ ...styles.escalaBtn, ...(resp === e.valor ? styles.escalaBtnActivo : {}) }}
...
const styles = {
  ...
  tag: { display: 'inline-block', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--accent)', background: 'var(--accent-dim)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: '100px', padding: '4px 12px', letterSpacing: '0.08em' },
  rehacerBanner: { background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '8px', padding: '10px 16px', fontSize: '13px', color: '#fbbf24' },
  card: { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '32px' },
  bloqIcon: { fontSize: '40px', marginBottom: '16px' },
  quizIcon: { width: '60px', height: '60px', background: 'var(--accent-dim)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' },
  ...
  pregCard: { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '20px 24px', transition: 'border-color 0.2s' },
  ...
  pregNum: { width: '26px', height: '26px', borderRadius: '50%', flexShrink: 0, background: 'var(--accent-dim)', border: '1px solid rgba(0,212,170,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--mono)' },
  ...
  escalaBtn: { width: '40px', height: '40px', borderRadius: '8px', flexShrink: 0, background: 'var(--surface2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 600, color: 'var(--muted2)', cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'var(--mono)' },
  escalaBtnActivo: { background: 'var(--accent)', border: '1px solid var(--accent)', color: '#020c08' },
  submitWrap: { padding: '8px 0 24px', display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center' },
  submitBtn: { padding: '13px 32px', background: 'var(--accent)', color: '#020c08', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--sans)', cursor: 'pointer' },
  submitBtnDisabled: { background: 'var(--border)', color: 'var(--muted)', cursor: 'not-allowed' },
  errorMsg: { fontSize: '13px', color: '#f87171', background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: '8px', padding: '10px 14px' },
  resultIcon:  { fontSize: '48px', marginBottom: '16px' },
  resultBadge: { display: 'flex', alignItems: 'center', gap: '20px', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' },
  ...
  nextStep: { display: 'flex', alignItems: 'flex-start', gap: '10px', background: 'var(--accent-dim)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: '8px', padding: '12px 14px', fontSize: '13px', color: 'var(--muted2)', marginBottom: '24px' },
  nextStepNum: { color: 'var(--accent)', fontWeight: 700, flexShrink: 0 },
  botonesWrap: { display: 'flex', gap: '12px', flexWrap: 'wrap' },
  irSistemaBtn: { padding: '12px 28px', background: 'var(--accent)', color: '#020c08', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--sans)', cursor: 'pointer' },
  rehacerBtn: { padding: '12px 20px', background: 'none', color: 'var(--muted2)', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '14px', fontFamily: 'var(--sans)', cursor: 'pointer' },
  ...
  modalTitle:     { fontSize: '16px', fontWeight: 600, color: 'var(--text)', margin: '0 0 8px' },

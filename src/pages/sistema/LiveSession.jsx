...
export default function LiveSession() {
  ...
  useEffect(() => {
    ...
    esRef.current = es
    es.onmessage = e => {
      try { setGazeData(JSON.parse(e.data)) } catch {}
    }
    return () => es.close()
  }, [])
    ...
    return (
      <div style={s.doneWrap}>
        <div style={s.doneCard}>
          ...
          <h2 style={s.doneTitle}>Sesión completada</h2>
          <p style={s.doneDesc}>
            Los datos oculares fueron capturados correctamente.
            Tu estilo cognitivo está siendo clasificado por el modelo XGBoost.
          </p>
          <div style={s.doneResult}>
            <div style={s.resultLabel}>Perfil detectado</div>
            <div style={s.resultValue}>Visual</div>
            <div style={s.resultConf}>Confianza: 84%</div>
          </div>
          <button style={s.histBtn} onClick={() => navigate('/historial')}>
            Ver en Historial →
          </button>
        </div>
      </div>
      ...
      <div style={s.root}>
        ...
        <div style={s.leftCol}>
          <div style={s.panelLabel}>
            ...
            <span style={s.camLabel}>Cámara</span>
          </div>
      ...
      {showCancel && (
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <p style={s.modalDesc}>Se perderán todos los datos capturados hasta ahora.</p>
...
const s = {
  ...
  timerValue: { fontFamily: 'var(--mono)', fontSize: '32px', fontWeight: 700, marginBottom: '10px' },
  progressBg: { height: '4px', background: 'var(--surface3)', borderRadius: '4px', overflow: 'hidden' },
  progressBar: { height: '100%', background: 'var(--accent)', borderRadius: '4px', transition: 'width 1s linear' },
  ...
  textTitulo: { fontSize: '16px', fontWeight: 600, marginBottom: '20px', color: 'var(--text)', letterSpacing: '-0.01em' },
  ...
  doneCard: { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '48px', maxWidth: '420px', textAlign: 'center' },
  doneIcon: { width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', color: 'var(--success)', margin: '0 auto 24px' },
  doneTitle: { fontSize: '22px', fontWeight: 600, marginBottom: '12px' },
  ...
  resultValue: { fontSize: '32px', fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--mono)' },
  resultConf: { fontSize: '12px', color: 'var(--muted)', marginTop: '4px' },
  histBtn: { width: '100%', padding: '12px', background: 'var(--accent)', border: 'none', borderRadius: '8px', color: '#020c08', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--sans)', cursor: 'pointer' },
